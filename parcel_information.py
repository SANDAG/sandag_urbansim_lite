...
sched_dev_sql = '''
    SELECT parcel_id, site_id, min(yr) as first_year, sum(res_units) as res_units
     FROM urbansim.urbansim.scheduled_development_do_not_use
     WHERE scenario = 1 and yr > 2016
	 group by site_id, parcel_id
	 order by parcel_id
'''
...
parcels_full = pd.merge(parcels.loc[:, parcels.columns != 'mgra_13'],sched_dev_df[['first_year','res_units','site_id']],
                        how='left',on='site_id')
parcels_full.loc[parcels_full.buildout < parcels_full.res_units, 'buildout'] = parcels_full.res_units
print('test')
    ...
    if target_number_of_units > 0:
        ...
        if parcels_to_choose.remaining_capacity.sum() < target_number_of_units:
            ...
            if len(capacity_site):
                ...
                selected_sites_parcels = shuffled_parcels[shuffled_parcels['site_id'].isin(merge_sch.site_id.values.
                                                                                           tolist())]
            ...
            parcels_picked = pd.concat([parcels_picked, sch_picked], sort=True)  # type: pd.DataFrame
...
def run_developer(households, hu_forecast, reg_controls, supply_fname, feasibility, year):
    ...
    subregional_targets = largest_remainder_allocation(control_totals_by_year, rem_current_hh)
    ...
    if year == 2017:
        ...
        subregional_targets = subregional_targets.drop(['target_units', 'jur_id'], 1)
        subregional_targets = pd.merge(subregional_targets, new_controls_by_year[['geo_id', 'target_units']],
                                       how='left', on='geo_id')
        # subregional_targets.units_added.fillna(0, inplace=True)
        # subregional_targets['target_units'] = subregional_targets.target_units - subregional_targets.units_added
        # subregional_targets = subregional_targets.drop(['units_added', 'jur_or_cpa_id'], 1)
    ...
    #             adu_builds = adu_builds.append(adu_prog[['parcel_id', 'capacity_type', 'units_added', 'source']], sort=True)
    #
...
def run_matching(run_match_output, regional_controls, households, hu_forecast):
    ...
    #     target_comparison_df = target_comparison_df.append(subregional_targets[['geo_id', 'yr', 'targets']], sort=True)
    #
    ...
    #         sr14cap = sr14cap.append(full_match_geo[['parcel_id', 'unit_change', 'year_simulation', 'capacity_type']], sort=True)
    #     if compatible_value == 0:

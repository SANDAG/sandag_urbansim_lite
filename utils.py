...
def parcel_picker(parcels_to_choose, target_number_of_units, name_of_geo, year_simulation):
    ...
    if target_number_of_units > 0:
        ...
        if parcels_to_choose.remaining_capacity.sum() < target_number_of_units:
            ...
            priority_parcels = pd.concat([previously_picked,capacity_sch, capacity_jur])  # type: pd.DataFrame
...
def run_developer(households, hu_forecast, reg_controls, supply_fname, feasibility, year):
    ...
    for jur in [14, 19]:
        ...
        if len(sch_cpas) and (sch_target > 0):
            adjust_sch = ((.9*jur_percent) / control_adjustments.loc[control_adjustments.geo_id.isin(sch_cpas)].control.sum())
            ...
            adjust_jur = ((.1*jur_percent) / control_adjustments.loc[~control_adjustments.geo_id.isin(sch_cpas)].control.sum())
